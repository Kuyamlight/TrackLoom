...
- 2026-06-29：已新增 `AppMidiNoteActions`，为首屏提供可测试的默认 MIDI 音符添加和删除末尾音符入口。JUCE 首屏可选择目标 MIDI 片段并追加一个 C4 四分音符，音符会按片段内已有音符顺序追加；也可删除该片段里时间位置最后的音符；时间线摘要会显示音符数量。CTest 覆盖成功添加、连续追加、缺失片段、音频片段拒绝、片段已满拒绝、删除末尾音符、空片段删除拒绝、缺失片段删除拒绝和音频片段删除拒绝。当前能力只是默认音符追加和末尾音符撤回，不等同于自由音符编辑、钢琴卷帘、量化、力度编辑或任意音符选择删除。
