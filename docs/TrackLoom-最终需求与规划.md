...
- 2026-07-02：已新增 `AppCommandPalette`，把 `AppMainMenuStatus` 展开为命令面板可用的应用层数据列表，保留菜单组名、标签、启用状态和稳定 command id，跳过分隔线与“暂无最近工程”等无 command id 的提示行，支持按菜单组名或标签进行轻量过滤，并可选择过滤结果中的第一个 enabled 命令但不执行命令。CTest 覆盖基础菜单展开、禁用命令保留、轨道组搜索、动态最近工程命令、提示行过滤、跳过 disabled 命令、缺失查询无选择和首个 enabled 命令选择。当前能力只是命令面板数据层和选择规则，不等同于可见命令面板弹窗、快捷键展示、自定义快捷键、完整编辑菜单或 AI 工具命令注册表。
- 2026-07-01 新增、2026-07-02 扩展：已新增 `AppCommandShortcuts`，把第一批平台无关文件快捷键映射到现有菜单 command id：`Ctrl+N` 新建工程、`Ctrl+O` 打开工程、`Ctrl+S` 保存、`Ctrl+Shift+S` 另存为；2026-07-02 起，`Ctrl+Z` 映射工程撤销，`Ctrl+Y` 和 `Ctrl+Shift+Z` 映射工程重做。JUCE `keyPressed` 现在把 `KeyPress` 转成 `AppShortcutChord`，再通过 `AppCommandDispatcher` 执行；Space 播放/停止切换继续走 `toggleAppPlayback`，因为它是切换语义，不等同于播放菜单中的独立播放和停止命令。CTest 覆盖文件快捷键、撤销/重做快捷键和未注册组合拒绝。当前能力只是第一批窗口内文件快捷键和基础撤销/重做快捷键，不等同于全局快捷键、完整编辑快捷键、快捷键自定义、可见命令面板 UI 或平台原生菜单快捷键展示。
