...
- 2026-07-01 新增、2026-07-03 扩展：已新增 `AppCommandShortcuts`，把第一批平台无关文件快捷键映射到现有菜单 command id：`Ctrl+N` 新建工程、`Ctrl+O` 打开工程、`Ctrl+S` 保存、`Ctrl+Shift+S` 另存为；2026-07-02 起，`Ctrl+Z` 映射工程撤销，`Ctrl+Y` 和 `Ctrl+Shift+Z` 映射工程重做。当前快捷键由同一份默认绑定表驱动，用于键盘触发和命令面板显示，避免触发规则与展示文案分离。JUCE `keyPressed` 现在把 `KeyPress` 转成 `AppShortcutChord`，再通过 `AppCommandDispatcher` 执行；Space 播放/停止切换继续走 `toggleAppPlayback`，因为它是切换语义，不等同于播放菜单中的独立播放和停止命令。2026-07-03 新增 `AppShortcutContext`，命令面板打开时，未被命令面板专门处理的全局快捷键不再触发保存、重新打开面板或其他主窗口命令。CTest 覆盖文件快捷键、撤销/重做快捷键、未注册组合拒绝、命令面板快捷键显示和命令面板打开上下文中的全局快捷键抑制。当前能力只是第一批窗口内文件快捷键、基础撤销/重做快捷键和命令面板输入焦点隔离，不等同于全局快捷键、完整编辑快捷键、快捷键自定义或平台原生菜单快捷键展示。
