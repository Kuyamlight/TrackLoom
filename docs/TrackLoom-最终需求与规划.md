...
- 2026-07-02：已新增 `AppCommandPalette`，把 `AppMainMenuStatus` 展开为命令面板可用的应用层数据列表，保留菜单组名、标签、启用状态和稳定 command id，跳过分隔线与“暂无最近工程”等无 command id 的提示行，并支持按菜单组名或标签进行轻量过滤。CTest 覆盖基础菜单展开、禁用命令保留、轨道组搜索、动态最近工程命令和提示行过滤。当前能力只是命令面板数据层，不等同于可见命令面板弹窗、快捷键展示、自定义快捷键、完整编辑菜单或 AI 工具命令注册表。
