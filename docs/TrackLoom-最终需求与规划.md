...
- 桌面壳已有应用层工程会话边界，可新建、保存、打开工程并管理当前路径、dirty 状态和首批命令历史；当前已把打开、保存、另存为接到 JUCE 文件选择器，并已提供首屏最近工程选择、打开入口、基础文件/播放菜单栏、基础菜单命令分发边界、第一批文件快捷键、轨道动作、轨道状态动作和默认 MIDI 片段创建/删除/重命名/复制命令历史基础，但尚未接入完整快捷键、完整编辑命令体系、完整撤销/重做 UI 或正式未保存确认弹窗；
...
- 2026-06-29 新增、2026-07-01 扩展：已新增 `AppMidiClipActions` 和 `AppTimelineStatus`，为首屏提供可测试的目标乐器轨选择、默认 MIDI 片段创建、目标 MIDI 片段删除、目标 MIDI 片段重命名、目标 MIDI 片段复制和只读时间线摘要。JUCE 首屏可选择乐器轨并创建一小节默认 MIDI 片段，新片段会追加到该轨已有片段之后；也可选择目标 MIDI 片段删除整个片段、重命名或复制到自身之后。默认 MIDI 片段创建现在通过核心 `AddClipCommand` 进入 `AppProjectSession` 命令历史，删除通过 `DeleteClipCommand` 进入命令历史，重命名通过 `RenameClipCommand` 进入命令历史，复制通过 `DuplicateClipCommand` 进入命令历史；撤销/重做会保留稳定 clip id，并恢复被删片段内的 MIDI 音符、旧片段名称或复制得到的新 note id。CTest 覆盖成功创建、追加位置、缺失轨道、非乐器轨拒绝、成功删除、缺失片段删除拒绝、音频片段删除拒绝、成功重命名、空名称拒绝、成功复制、空时间线、片段行摘要，以及创建/删除/重命名/复制撤销重做。当前仍未实现正式时间线拖拽、完整 MIDI 片段编辑命令历史、钢琴卷帘或播放控制接入。
...
- 2026-07-01：已新增 `AppMainMenu`，把文件菜单、播放菜单和最近工程动态菜单项整理成可测试的应用层主菜单快照。JUCE 首屏已接入 `MenuBarComponent`，文件菜单可触发新建工程、打开工程、保存、另存为和最近工程打开，播放菜单可触发播放、停止和回到开头；菜单项启用状态读取现有应用会话、播放运行态和最近工程列表，实际命令继续复用原有按钮动作。CTest 覆盖文件/播放菜单分组、稳定 command id、空最近工程禁用行、播放中启用状态和最近工程 command id 往返。当前能力只是基础文件/播放菜单栏，不等同于完整菜单体系、全局快捷键、菜单栏里的全部编辑命令、正式未保存确认弹窗或平台原生命令路由。
- 2026-07-01：已新增 `AppCommandDispatcher`，把基础菜单 command id 解析成稳定 `AppCommandKind`，并通过注入的处理函数执行新建、打开、保存、另存为、播放、停止、回到开头和最近工程打开。JUCE 菜单点击现在只构造命令处理回调表并调用分发器，不再在 `menuItemSelected` 中复制普通命令和最近工程动态命令的映射。CTest 覆盖普通命令分发、最近工程编号传递、未知命令拒绝和缺失处理函数拒绝。当前能力只是文件/播放菜单的基础命令分发边界，不等同于完整快捷键体系、完整编辑菜单、命令面板、撤销重做菜单或 AI 工具命令注册表。
- 2026-07-01：已新增 `AppCommandShortcuts`，把第一批平台无关文件快捷键映射到现有菜单 command id：`Ctrl+N` 新建工程、`Ctrl+O` 打开工程、`Ctrl+S` 保存、`Ctrl+Shift+S` 另存为。JUCE `keyPressed` 现在把 `KeyPress` 转成 `AppShortcutChord`，再通过 `AppCommandDispatcher` 执行；Space 播放/停止切换继续走 `toggleAppPlayback`，因为它是切换语义，不等同于播放菜单中的独立播放和停止命令。CTest 覆盖文件快捷键映射和未注册组合拒绝。当前能力只是第一批窗口内文件快捷键，不等同于全局快捷键、完整编辑快捷键、快捷键自定义、命令面板或平台原生菜单快捷键展示。
- 2026-07-01：已在 `AppProjectSession` 中新增应用层工程命令历史入口，可执行核心 `Command`，查询、撤销和重做命令历史；成功命令才标脏并进入历史，失败命令不标脏不入栈，新建或打开工程会清空历史。为避免旧直接编辑入口造成错误撤销，`editProject()` 现在会清空旧历史。首批已把 `AppTrackActions` 的轨道创建、删除、重命名和上下移动迁移到核心命令执行；`AppTrackStateActions` 的静音、独奏、禁用和隐藏也已迁移到核心状态命令；`AppMidiClipActions` 的默认 MIDI 片段创建、删除、重命名和复制已迁移到核心片段命令。因此这些轨道动作、轨道状态动作和默认 MIDI 片段创建/删除/重命名/复制可以通过会话历史撤销/重做。CTest 覆盖命令执行、失败命令、历史重置、直接编辑清空历史、轨道创建撤销/重做、静音撤销/重做、隐藏撤销/重做、MIDI 片段创建撤销/重做、MIDI 片段删除撤销/重做、MIDI 片段重命名撤销/重做和 MIDI 片段复制撤销/重做。当前能力只是命令历史基础，不等同于完整 Undo/Redo 菜单、快捷键、命令面板或所有音频/MIDI 片段和音符动作都已可撤销。
