...
- 2026-06-29 新增、2026-06-30 扩展 `AppMidiNoteActions`，为首屏提供可测试的默认 MIDI 音符添加、删除末尾音符、末尾音符半音升降、末尾音符力度增减和末尾音符长度增减入口。JUCE 首屏可选择目标 MIDI 片段并追加一个 C4 四分音符，音符会按片段内已有音符顺序追加；也可删除该片段里时间位置最后的音符，把末尾音符升高/降低一个 MIDI note number，按 8 调整末尾音符 velocity，或按十六分音符 tick 延长/缩短末尾音符。音高调整到 0-127 范围外、力度调整到 1-127 范围外、长度短于十六分音符、长度超出片段右边界、空片段、缺失片段和音频片段会被拒绝且不标脏工程。CTest 覆盖成功添加、连续追加、缺失片段、音频片段拒绝、片段已满拒绝、删除末尾音符、升高/降低末尾音符、增强/减弱末尾音符力度、延长/缩短末尾音符长度、音高上下界拒绝、力度上下界拒绝、长度上下界拒绝、空片段音高/力度/长度调整拒绝、缺失片段力度/长度调整拒绝、音频片段力度/长度调整拒绝和删除失败路径。当前能力只是默认音符追加、末尾音符撤回、末尾音符音高微调、末尾音符力度步进微调和末尾音符长度步进微调，不等同于自由音符编辑、钢琴卷帘、量化、力度曲线、任意音符力度编辑、任意音符长度编辑或任意音符选择删除。
