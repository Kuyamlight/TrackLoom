...
- 桌面壳已有应用层工程会话边界，可新建、保存、打开工程并管理当前路径、dirty 状态和首批命令历史；当前已把打开、保存、另存为接到 JUCE 文件选择器，并已提供首屏最近工程选择、打开入口、基础文件/播放菜单栏、基础菜单命令分发边界、第一批文件快捷键、轨道动作、轨道状态动作和默认 MIDI 片段创建/删除命令历史基础，但尚未接入完整快捷键、完整编辑命令体系、完整撤销/重做 UI 或正式未保存确认弹窗；
...
- 2026-06-29 新增、2026-07-01 扩展：已新增 `AppMidiClipActions` 和 `AppTimelineStatus`，为首屏提供可测试的目标乐器轨选择、默认 MIDI 片段创建、目标 MIDI 片段删除和只读时间线摘要。JUCE 首屏可选择乐器轨并创建一小节默认 MIDI 片段，新片段会追加到该轨已有片段之后；也可选择目标 MIDI 片段并删除整个片段。默认 MIDI 片段创建现在通过核心 `AddClipCommand` 进入 `AppProjectSession` 命令历史，删除通过 `DeleteClipCommand` 进入命令历史，撤销/重做会保留稳定 clip id 并恢复被删片段内的 MIDI 音符。CTest 覆盖成功创建、追加位置、缺失轨道、非乐器轨拒绝、成功删除、缺失片段删除拒绝、音频片段删除拒绝、空时间线、片段行摘要，以及创建/删除撤销重做。当前仍未实现正式时间线拖拽、完整 MIDI 片段编辑命令历史、钢琴卷帘或播放控制接入。
...
- 2026-07-01：已在 `AppProjectSession` 中新增应用层工程命令历史入口，可执行核心 `Command`，查询、撤销和重做命令历史；成功命令才标脏并进入历史，失败命令不标脏不入栈，新建或打开工程会清空历史。为避免旧直接编辑入口造成错误撤销，`editProject()` 现在会清空旧历史。首批已把 `AppTrackActions` 的轨道创建、删除、重命名和上下移动迁移到核心命令执行；`AppTrackStateActions` 的静音、独奏、禁用和隐藏也已迁移到核心状态命令；`AppMidiClipActions` 的默认 MIDI 片段创建和删除已迁移到核心片段命令。因此这些轨道动作、轨道状态动作和默认 MIDI 片段创建/删除可以通过会话历史撤销/重做。CTest 覆盖命令执行、失败命令、历史重置、直接编辑清空历史、轨道创建撤销/重做、静音撤销/重做、隐藏撤销/重做、MIDI 片段创建撤销/重做和 MIDI 片段删除撤销/重做。当前能力只是命令历史基础，不等同于完整 Undo/Redo 菜单、快捷键、命令面板或所有音频/MIDI 片段和音符动作都已可撤销。
