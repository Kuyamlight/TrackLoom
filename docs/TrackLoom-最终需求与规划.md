...
- 2026-07-02 起，空音频片段创建、重命名和删除已接入同一会话命令历史，可撤销/重做并保留稳定 clip id；音频片段复制、拆分、跨轨移动、一拍移动和片头/片尾编辑仍未完成命令历史迁移；
...
- 2026-07-02：`AppAudioClipActions` 中默认空音频片段创建、重命名和删除已迁移到 `AppProjectSession` 命令历史：创建通过 `AddClipCommand`，重命名通过 `RenameClipCommand`，删除通过 `DeleteClipCommand`。撤销/重做会保留同一个 clip id，并恢复片段名称、轨道、类型、起点和长度。CTest 新增并通过音频片段创建、重命名、删除撤销/重做覆盖。当前只有这三个音频片段动作完成命令历史迁移；复制、中点拆分、跨轨移动、一拍移动和片头/片尾编辑仍需后续按红绿灯测试逐步迁移，不能提前对外宣称所有音频片段动作都可撤销。
...
- 2026-07-02：音频片段动作开始接入同一命令历史边界。默认空音频片段创建通过 `AddClipCommand`，音频片段重命名通过 `RenameClipCommand`，音频片段删除通过 `DeleteClipCommand`，因此这三项可通过会话历史撤销/重做，并保留稳定 clip id。CTest 覆盖音频片段创建、重命名和删除撤销/重做。当前仍不代表音频片段复制、拆分、移动、片头/片尾编辑或 MIDI 音符动作已经完成命令历史迁移。
