...
- 2026-07-06 起，`AppCommandShortcuts` 已增加应用层自定义快捷键活动表基础：从默认绑定生成活动表，接受不冲突覆盖，拒绝并报告冲突覆盖，并允许给无默认快捷键的命令新增绑定；当前仍没有设置界面、快捷键持久化、完整编辑快捷键或平台原生菜单快捷键展示；
...
- 2026-07-06：已扩展 `AppCommandShortcuts`，新增 `customizeAppShortcutBindings`、`AppShortcutConflict` 和按活动绑定表查询的 `appCommandIdForShortcut(chord, bindings)`；应用层现在可以从默认快捷键表生成活动表，非冲突用户覆盖会替换同一命令旧绑定，冲突覆盖会报告现有命令和请求命令并保留原绑定，也能为无默认快捷键的命令新增绑定。CTest 覆盖替换默认绑定、冲突拒绝和新增无默认绑定命令。当前能力只是自定义快捷键的数据层基础，不代表已经有设置界面、保存到本地设置、完整快捷键体系或平台原生菜单快捷键展示。
