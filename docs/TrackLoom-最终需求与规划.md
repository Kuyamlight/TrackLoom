...
- 桌面壳已有应用层工程会话边界，可新建、保存、打开工程并管理当前路径、dirty 状态和首批命令历史；当前已把打开、保存、另存为接到 JUCE 文件选择器，并已提供首屏最近工程选择、打开入口、基础文件/编辑/播放菜单栏、基础菜单命令分发边界、第一批文件快捷键、基础撤销/重做快捷键、轨道动作、轨道状态动作、默认 MIDI 片段创建/删除/重命名/复制/拆分/一拍移动/跨轨移动/片尾缩短/延长/片头缩短/延长命令历史基础、空音频片段外壳创建/删除/重命名/复制/拆分/跨轨移动/一拍移动/片尾缩短/片尾延长/片头缩短/片头延长命令历史基础，以及 MIDI 音符创建/删除/复制/音高/力度/长度/起点微调命令历史基础；基础编辑菜单和快捷键已能触发工程撤销/重做，但尚未接入完整快捷键、完整编辑命令体系、命令面板或正式未保存确认弹窗；
